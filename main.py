...
def main(n: int) -> Tuple[List[int], int]:
    ...
    global NUM_GRID
    height, width = NUM_GRID.shape

    # Check horizontal lines
    for y in range(height):
        for x in range(width-n):
            this_seq = NUM_GRID[y, x:x+n]
            this_p = reduce(operator.mul, this_seq)
            if this_p > p:
                seq, p = this_seq, this_p

    # Check vertical lines
    for y in range(height-n):
        for x in range(width):
            this_seq = NUM_GRID[y:y+n, x]
            this_p = reduce(operator.mul, this_seq)
            if this_p > p:
                seq, p = this_seq, this_p

    # Check diagonal lines like this -> \
    for y in range(height-n):
        for x in range(width-n):
            # Construct seq by stepping diagonally
            this_seq = [NUM_GRID[y+j, x+j] for j in range(n)]
            this_p = reduce(operator.mul, this_seq)
            if this_p > p:
                seq, p = this_seq, this_p
...
if __name__ == '__main__':
    ...
    factors, product = main(num)
    print('Largest product of {} adjacent elements in grid:'.format(num))
